...
//static const char **password = {"secret1", "secret2", "secret3"};
static const char *password = "enjoy";
...
{	
    ...
    for(int i=0; i < MAX_PEOPLE; i++) {
    	char * pw = password[i];
    	if(strcmp(password_text, pw) == 0) {
    		printf("Access granted!\n");
    		g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(socket_chat), NULL);
    		break;
    	}	
        else
...
int main(int argc, char *argv[])
{
    ...
    g_signal_connect(G_OBJECT(ok_button), "clicked", G_CALLBACK(socket_chat), NULL);
    if(strcmp(password_text, password) == 0) {
    	printf("Access granted!\n");
